...
export class StatusRenderer {
  ...
  private detectColorSupport(): boolean {
    ...
    const term = process.env['TERM'] || '';
    return term.includes('color') || term.includes('256') || term.includes('truecolor');
  }
  ...
  getColor(status: TaskStatus): string {
    ...
    const colorMap = this.options.useHighContrast ? HIGH_CONTRAST_COLORS : STATUS_COLORS;
  ...
  getLegendText(): string {
    ...
    const items = statuses.map(status => {
      const glyph = this.getGlyph(status);
      const description = STATUS_GLYPHS[status]?.description.split(' - ')[0] || status;
