...
export class ColorIntegrationSystem {
   ...
   */
  private applyBackgroundHighlighting(line: string, relationship: DependencyRelationship): string {
   ...
   */
  private applyForegroundHighlighting(line: string, relationship: DependencyRelationship): string {
    // Skip 'none' relationship
    if (relationship === 'none') return line;
   ...
   */
  private applyForegroundColorPreservingExisting(line: string, fgColor: string): string {
   ...
   */
  private applyTextStyling(line: string, relationship: DependencyRelationship): string {
    // Skip 'none' relationship
    if (relationship === 'none') return line;
  ...
  getLegendText(): string {
    const relationships = [
      { type: 'blocking-pending', desc: '⚠ Blocking (pending)', sample: 'red bg' },
      { type: 'blocking-completed', desc: '✓ Blocking (done)', sample: 'green bg' },
      { type: 'dependent', desc: '← Dependent', sample: 'blue bg' },
      { type: 'related', desc: '~ Related', sample: 'yellow bg' }
    ];

    return relationships
      .map(r => `${r.desc}: ${r.sample}`)
      .join('  ');
  }
  ...
  testColorCombinations(): string[] {
    ...
    issues.push('Manual testing required: Status glyphs on dependency backgrounds');
 ...
 */
export const defaultColorIntegration = ColorIntegrationSystem.createBackgroundOnly(); 