...
import type { ContextEvent, ContextTracker } from "./context-tracker.js";
...
export class FirstRunDetector {
	...
	private recordPaneVisit(context: PaneContext): void {
		...
		if (process.env["DEBUG_ONBOARDING"]) {
			...
			console.error(
				`DEBUG: Recorded visit to ${context} (count: ${visit.visitCount})`,
			);
		}
	}
	...
	private async checkAndTriggerTooltips(context: PaneContext): Promise<void> {
		...
		for (const binding of tooltipBindings) {
			...
			if (process.env["DEBUG_ONBOARDING"]) {
				console.error(
					`DEBUG: Triggered tooltip for ${binding.key} in ${context} (visit: ${visit.visitCount})`,
				);
			}
		}
	}
	...
	getVisitInfo(
		context?: PaneContext,
	): PaneVisitEntry | Map<PaneContext, PaneVisitEntry> {
		if (context) {
			return (
				this.visits.get(context) || {
					context,
					visitCount: 0,
					firstVisit: 0,
					lastVisit: 0,
	...
	isPaneEligibleForOnboarding(context: PaneContext): boolean {
		const visit = this.visits.get(context);
		return visit
			? visit.onboardingActive &&
					visit.visitCount <= this.config.maxTooltipVisits
			: true;
	}
	...
	getStats(): {
		...
		const activeOnboardingPanes = visits.filter(
			(v) => v.onboardingActive,
		).length;
