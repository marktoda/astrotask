...
export class KeyMapRegistry extends EventEmitter {
	...
	public register(binding: EnhancedKeyBinding): string {
		...
		if (this.hasKeyConflict(binding.key, binding.context, id)) {
			throw new Error(
				`Key conflict: "${binding.key}" is already bound in context "${binding.context}"`,
			);
		}
	...
	public query(options: KeyBindingQuery = {}): KeyBindingRegistryEntry[] {
		...
		if (options.context) {
			...
			if (contextBindings) {
				results = Array.from(contextBindings)
					...
					.filter(
						(entry): entry is KeyBindingRegistryEntry => entry !== undefined,
					);
			}
		} else {
		...
		if (options.category) {
			results = results.filter(
				(entry) => entry.binding.category === options.category,
			);
		}
		...
		if (options.priority) {
			results = results.filter(
				(entry) => entry.binding.priority === options.priority,
			);
		}
	...
	public getMetrics(): {
		...
		const averageTimeToFirstUse =
			timeToFirstUseValues.length > 0
				? timeToFirstUseValues.reduce((a, b) => a + b, 0) /
					timeToFirstUseValues.length
				: 0;
