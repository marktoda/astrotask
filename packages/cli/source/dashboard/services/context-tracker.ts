...
export class ContextTracker {
  ...
  private readonly contextMapping: Map<DashboardStore["activePanel"], PaneContext> = new Map([
  ...
  private addToHistory(event: ContextEvent): void {
    ...
    if (this.state.navigationHistory.length > this.MAX_HISTORY_LENGTH) {
      this.state.navigationHistory = this.state.navigationHistory.slice(-this.MAX_HISTORY_LENGTH);
    }
  }
  ...
  public shouldShowFooterHints(): boolean {
    ...
    return this.state.isIdle || this.getTimeSinceLastNavigation() >= this.IDLE_THRESHOLD_MS;
  }
  ...
  public getNavigationPatterns(): {
    ...
    recentTransitions: Array<{ from: PaneContext; to: PaneContext; count: number }>;
  } {
    ...
    const recentTransitions = Array.from(transitions.entries()).map(([key, count]) => {
