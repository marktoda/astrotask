...
import type { FirstRunDetector, TooltipTriggerEvent } from "../../services/first-run-detector.js";
...
export class GhostTextRenderer {
  ...
  private initialize(): void {
    ...
    this.firstRunDetector.addEventListener(this.handleTooltipTrigger.bind(this));
  ...
  private handleTooltipTrigger(event: TooltipTriggerEvent): void {
    if (process.env["DEBUG_ONBOARDING"]) {
      console.error(`DEBUG: Ghost text trigger for ${event.binding.key} in ${event.context}`);
    }
  ...
  private async showTooltip(event: TooltipTriggerEvent): Promise<void> {
    ...
    if (!position) {
      if (process.env["DEBUG_ONBOARDING"]) {
        console.error(`DEBUG: Could not calculate position for tooltip in ${event.context}`);
      }
      return;
    }
    ...
    if (process.env["DEBUG_ONBOARDING"]) {
      console.error(`DEBUG: Showing ghost text for ${event.binding.key} at position ${JSON.stringify(position)}`);
    }
  }
   ...
   */
  private calculateTooltipPosition(context: PaneContext): { top: number; left: number; width: number } | null {
    const state = this.store.getState();
    
    // Get the active panel's focused element position
    let focusedElement: blessed.Widgets.BlessedElement | null = null;
    let panelBounds: { top: number; left: number; width: number; height: number } | null = null;
   ...
   */
  private createTooltipElement(binding: EnhancedKeyBinding, position: { top: number; left: number; width: number }): blessed.Widgets.BoxElement {
    // Format the keybinding text
    const keyText = this.formatKeyText(binding.key);
    const descText = binding.description || binding.action?.toString().slice(0, 20) || "";
  ...
  private setupFocusChangeHandler(): void {
    ...
    this.store.subscribe((state, prevState) => {
      if (state.activePanel !== prevState.activePanel && this.config.autoHideOnFocusChange) {
        this.hideAllTooltips();
      }
    });
  }
   ...
   */
  private findFocusedListElement(_panel: "tree" | "sidebar" | "details"): blessed.Widgets.BlessedElement | null {
   ...
   */
  private getPanelBounds(panel: "tree" | "sidebar" | "details"): { top: number; left: number; width: number; height: number } | null {
