  ...
  private setContent() {
    const sections: Section[] = [
      {
        title: "Global",
        bindings: [
          { keys: ["q", "Ctrl+c"], description: "Quit application (double tap)" },
      ...
      },
      {
        ...
        bindings: [
          ...
          { keys: ["add \"title\" under ID"], description: "Add task under parent" },
    ...
    sections.forEach(section => {
      ...
      section.bindings.forEach(binding => {
        ...
        lines.push(`  {cyan-fg}${keys}{/}${" ".repeat(Math.max(0, padding))}${binding.description}`);
      });
