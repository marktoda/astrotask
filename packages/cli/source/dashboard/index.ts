...
async function main() {
  try {
    ...
    try {
      if (screen.program && typeof (screen.program as any).keypad === 'function') {
      ...
      console.warn('Warning: Could not enable enhanced input mode:', programError.message);
    }

    // Debug key handling (enable with DEBUG_KEYS=1)
    if (process.env['DEBUG_KEYS']) {
      screen.on('keypress', (_ch, key) => {
        console.error(`DEBUG: Key pressed - full: "${key.full}", sequence: ${JSON.stringify(key.sequence)}, name: "${key.name}"`);
      });
    }
