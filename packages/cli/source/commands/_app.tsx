...
export default function App({ Component, commandProps }: AppProps) {
	...
	React.useEffect(() => {
		...
		createDatabase(dbOptions)
			.then((store: any) => {
				console.log(`Initialized database at: ${store.pgLite.dataDir} (with locking)`);
			...
			.catch((err: any) => {
				...
				if (err.message?.includes('Database is currently in use')) {
					...
					console.error("   Please wait a moment and try again, or check if the MCP server is running.");
				} else {
