...
describe('Database Migration System', () => {
  ...
  it('should handle migration errors gracefully', async () => {
    ...
    // Create a completely isolated temp directory to run test from
    // This ensures fallback paths won't find real migration files
    const isolatedTestDir = join(tmpdir(), `astrolabe-isolated-${Date.now()}`);
    const originalCwd = process.cwd();
    
    try {
      // Change to isolated directory to break fallback logic
      mkdirSync(isolatedTestDir, { recursive: true });
      process.chdir(isolatedTestDir);
      
      // Try to run migrations with invalid path and no fallbacks available
      await expect(
        runMigrations(connection, {
          migrationsFolder: '/completely/invalid/path/that/does/not/exist',
          verbose: false,
        })
      ).rejects.toThrow();
    } finally {
