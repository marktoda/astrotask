...
export function validateAcceptanceCriteria(acceptanceCriteria: ContextSlice[]): {
  ...
  for (const criteria of acceptanceCriteria) {
    ...
    const hasActionVerb = /\b(returns?|loads?|responds?|validates?|accepts?|rejects?|handles?|displays?|completes?)\b/i.test(criteria.title);
    if (!hasActionVerb) {
      suggestions.push(`Consider using action verbs in "${criteria.title}" for clearer expectations`);
    }
    ...
    const hasVagueLanguage = /\b(works?|good|bad|nice|better|properly|correctly)\b/i.test(criteria.title + ' ' + (criteria.description || ''));
    if (hasVagueLanguage) {
      suggestions.push(`Avoid vague language in "${criteria.title}" - be specific about expected behavior`);
    }
  }
...
export function assessAcceptanceCriteriaCompletion(acceptanceCriteria: ContextSlice[]): {
  ...
  const potentiallyMet = acceptanceCriteria.filter(criteria => {
    ...
    return text.includes('✓') || text.includes('complete') || text.includes('done') || text.includes('met');
  }).length;
 ...
 */
export function suggestAcceptanceCriteria(taskTitle: string, taskDescription?: string): Array<{
  title: string;
  description: string;
}> {
  ...
  if (combined.includes('api') || combined.includes('endpoint')) {
    suggestions.push({
      ...
      description: 'Endpoint returns 200 for success, 400 for bad requests, 404 for not found, 500 for server errors'
    });
    suggestions.push({
      title: 'API validates input data',
      description: 'Endpoint properly validates required fields and returns descriptive error messages for invalid input'
    });
  }
  
  // UI/Frontend tasks
  if (combined.includes('ui') || combined.includes('frontend') || combined.includes('page') || combined.includes('component')) {
    suggestions.push({
      title: 'UI renders correctly on different screen sizes',
      description: 'Component displays properly on mobile (320px), tablet (768px), and desktop (1024px+) viewports'
    });
    suggestions.push({
      title: 'UI is accessible to screen readers',
      description: 'All interactive elements have proper ARIA labels and can be navigated using keyboard only'
    });
  }
  ...
  if (combined.includes('auth') || combined.includes('login') || combined.includes('user')) {
    suggestions.push({
      ...
      description: 'System returns appropriate error message for wrong username/password without revealing which field is incorrect'
    });
    suggestions.push({
      title: 'Sessions expire appropriately',
      description: 'User sessions expire after configured timeout period and require re-authentication'
    });
  }
  ...
  if (combined.includes('performance') || combined.includes('speed') || combined.includes('load')) {
    suggestions.push({
      ...
      description: 'Operation completes within acceptable time limits (specify exact timing requirements)'
    });
  }
  
  // Database tasks
  if (combined.includes('database') || combined.includes('data') || combined.includes('migration')) {
    suggestions.push({
