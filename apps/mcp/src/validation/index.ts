...
export const sanitizedString = (options: {
  ...
  return z.string()
    .min(allowEmpty ? 0 : Math.max(min, 1), `Must be at least ${allowEmpty ? 0 : Math.max(min, 1)} characters`)
 ...
 */
export const taskStatusSchema = z.enum(['pending', 'in-progress', 'done', 'cancelled'])
...
export const createTaskSchema = z.object({
  title: sanitizedString({ min: 1, max: 200 })
    .describe('Task title'),
  description: sanitizedString({ max: 2000, allowEmpty: true }).optional()
  ...
  prd: sanitizedString({ max: 10000, allowEmpty: true }).optional()
    .describe('Product Requirements Document content'),
  contextDigest: sanitizedString({ max: 1000, allowEmpty: true }).optional()
    .describe('Context digest for the task'),
  tags: z.array(sanitizedString({ min: 1, max: 50 })).max(10).optional()
...
export const listTasksSchema = z.object({
  ...
  parentId: uuidSchema.optional()
    .describe('Filter tasks by parent ID (for subtasks)'),
  includeSubtasks: z.boolean().default(false)
    .describe('Include subtasks in the response'),
  priority: prioritySchema.optional()
    .describe('Filter tasks by priority'),
  tags: z.array(sanitizedString({ min: 1, max: 50 })).max(5).optional()
    .describe('Filter tasks by tags (max 5)'),
  search: sanitizedString({ max: 100, allowEmpty: true }).optional()
...
export const deleteTaskSchema = z.object({
  ...
  cascade: z.boolean().default(true)
    .describe('Delete all subtasks as well')
});
...
export const getTaskContextSchema = z.object({
  id: uuidSchema
    .describe('Task ID to get context for'),
  includeAncestors: z.boolean().default(true)
    .describe('Include ancestor tasks'),
  includeDescendants: z.boolean().default(true)
    .describe('Include descendant tasks'),
...
export const envSchema = z.object({
  ...
  MCP_SERVER_PORT: z.string().transform(val => parseInt(val, 10)).pipe(z.number().int().min(1).max(65535)).default('3000'),
